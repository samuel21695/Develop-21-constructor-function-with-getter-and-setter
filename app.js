class User {
  ...
  constructor(username, email) {
    ...
    // 객체의 값을 부여하는데, '적절한' 값이어야만 값으로 대입되도록
    // setter 함수를 통해 return 처리를 분배해서 진행한다.
    // 참인 경우 은닉화된 키 값인 _email에 값을 대입하고,
    // 거짓인 경우 null을 대입한다.
    ...
    if (this.validateEmail(email)) {
      ...
      this._email = null; // null 데이터타입의 값어치
    }
  }

  // '가져오는', '읽어오는' 작업을 할 수 있는 getter 기능으로
  // 읽기 전용(read only) 프로퍼티를 정의한다.
  ...
  showProfile() {
    console.log(`Usesrname: ${this.username}. Email: ${this._email}`);
  }
}

// 에제 호출
const userKong = new User("samuelso", "samuelso@example.com");
